  ...
  for (const col of collections) {
    ...
    const variables = colVars.map((v) => {
      ...
      for (const mode of col.modes) {
        valuesByMode[mode.name] = resolveVariableValue(v, mode.modeId);
      }
  ...
  try {
    ...
    for (var ei = 0; ei < entries.length; ei++) {
      ...
      for (var mi = 0; mi < col.modes.length; mi++) {
        valuesByMode[col.modes[mi].name] = resolveVariableValue(v, col.modes[mi].modeId);
      }
      var currentData = {
        id: v.id,
        name: v.name,
...
async function dTextRow(parent, style) {
  ...
  await addB("$font-size-" + Math.round(style.fontSize));
  if (style.lineHeight && style.lineHeight.unit !== "AUTO") {
    await addB("$line-height-" + Math.round(style.lineHeight.value));
  }
  await addB("$font-family-" + style.fontFamily.toLowerCase().replace(/\s+/g, "-"));
  await addB("$font-weight-" + style.fontStyle.toLowerCase().replace(/\s+/g, "-"));
  if (style.letterSpacing) {
    var ls = style.letterSpacing.value;
    await addB("$letter-spacing-" + (ls === 0 ? "00" : ls.toFixed(1)));
  }
